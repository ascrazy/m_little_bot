...
export function toParagraphs(inputString: string, entities: MessageEntity[]): Paragraph[] {
...
function splitParagraphs(inputString: string, entities: MessageEntity[]): MessageParagraph[] {
...
function selectRelevantEntities(paragraph: MessageParagraph, entities: MessageEntity[]): MessageEntity[] {
    const relevantEntities = entities
        .filter(entity => entity.offset >= paragraph.startIndex && entity.offset <= paragraph.startIndex + paragraph.text.length)
        .map(entity => {
            return {
                ...entity,
